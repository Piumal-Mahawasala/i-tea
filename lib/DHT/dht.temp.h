...
#define HUMIDITY_CHANGE_THRESHOLD 0.2 // Humidity change threshold for triggering the alarm (adjust as needed)
...
void temp_alarm()
{
  ...
  oled.setCursor(0,2);
    ...
    if (currentMillis - previousMillis >= SAMPLE_INTERVAL)
    {
        ...
        if (isHumidityRising(currentHumidity, previousHumidity) && isAlarmActive == false)
        {
            full();
            displayTime(counter);
...
bool isHumidityRising(float currentHumidity, float previousHumidity)
{
   return (currentHumidity - previousHumidity) >= HUMIDITY_CHANGE_THRESHOLD;
